...
module cu_interrupt_tb;
    ...
    initial begin
        ...
        // Section 3: INTR_CHECK ignores interrupt when I_flag=0
        intr_pending = 1;
        ...
        // Section 6: WAIT — idles until intr_pending=1, then exits
        @(negedge clk);  // LOAD_INSTR
        ...
        // Section 7: WAIT with I_flag=0 — stays stuck, never services IRQ
        reset_to_decode();
        opcode = OP_WAIT;
        ...
        // Section 8: nested interrupt blocked during ISR save (I_flag=0 in ISR)
        reset_to_decode();
        opcode = OP_EI;
        @(negedge clk);  // EI_1
        @(negedge clk);  // LOAD_ADDR
        ...
        @(negedge clk);  // LOAD_ADDR
        @(negedge clk);  // LOAD_INSTR
        @(negedge clk);  // INTR_CHECK (I_flag=0) → DECODE
